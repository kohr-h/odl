...
class WaveletTransform(Operator):
    ...
    def __init__(self, domain, nscales, wbasis, mode, axes=None):
...
-dwt-max-level>`_ .
        ...
        if axes is not None:
            if len(axes) != domain.ndim:
                ...
                self.size_list = coeff_size_list_axes(
                    domain.shape, self.wbasis, self.mode, self._axes)
    ...
    def _call(self, x):
        ...
        if self._axes is None:
            ...
            coeff_dict = pywt.dwtn(x, self.wbasis, self.mode, self._axes)
    ...
    def inverse(self):
        ...
        return WaveletTransformInverse(
            ...
            mode=self.mode, axes=self._axes)
...
class WaveletTransformInverse(Operator):
    ...
    def __init__(self, range, nscales, wbasis, mode, axes=None):
        """Initialize a new instance.
...
-dwt-max-level>`_ .
        ...
        if axes is not None:
            if len(axes) != range.ndim:
                ...
                self.size_list = coeff_size_list_axes(
                    range.shape, self.wbasis, self.mode, self._axes)
    ...
    def _call(self, coeff):
        ...
        if self._axes is None:
            ...
            x = pywt.idwtn(coeff_dict, self.wbasis, self.mode, self._axes)
            return x
