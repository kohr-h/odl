...
class WaveletTransform(Operator):
    ...
    def __init__(self, domain, wbasis, mode, nscales=None, axes=None):
...
-dwt-max-level>`_ .
        ...
        if axes is not None:
            ...
            self.size_list = coeff_size_list(
                domain.shape, self.wbasis, self.mode, self.nscales, axes=None)
    ...
    def _call(self, x):
        ...
        if self.axes is None:
            ...
            if self.domain.ndim == 3:
                coeff_list = pywt.wavedecn(x, self.wbasis, self.mode,
                                           self.nscales)
                coeff_arr = pywt_list_to_array(coeff_list, self.size_list)

                return self.range.element(coeff_arr)
        else:
            coeff_dict = pywt.dwtn(x, self.wbasis, self.mode, self.axes)
    ...
    def adjoint(self):
        ...
        if self.is_orthogonal:
            if self.wbasis.dec_len == 2:
                output = self.inverse
                output /= self.domain.cell_volume
                return output
            elif self.mode == 'zero' or self.mode == 'periodization':
                output = self.inverse
                output /= self.domain.cell_volume
                return output
            elif (self.mode == 'constant' or
                  self.mode == 'symmetric' or
                  self.mode == 'periodic'):
                output = WaveletTransformAdjoint(
                    ran=self.domain, wbasis=self.wbasis, mode=self.mode,
            ...
            if self.wbasis.dec_len == 2:
                ...
                output /= self.domain.cell_volume
            elif self.mode == 'zero' or 'periodization':
                adjoint_name = self.wbasis.name.replace('bior', 'rbio')
                wbasis_adjoint = pywt.Wavelet(adjoint_name)
                output = WaveletTransformInverse(
                    ran=self.domain, wbasis=wbasis_adjoint, mode=self.mode,
    ...
    def inverse(self):
        ...
        return WaveletTransformInverse(
            range=self.domain, wbasis=self.wbasis, mode=self.mode,
            nscales=self.nscales, axes=self.axes)
        ...
        if axes is not None:
            ...
            if self.nscales > max_level:
                raise ValueError('Cannot use more than {} scaling levels, '
                                 ...
                                 ''.format(max_level, self.nscales))
            self.size_list = coeff_size_list(
                range.shape, self.wbasis, self.mode, self.nscales, axes=None)
    ...
    def _call(self, coeff):
        ...
        if self.axes is None:
            if len(self.range.shape) == 1:
                ...
                coeff_list = array_to_pywt_list(coeff, self.size_list)
                x = pywt.waverecn(coeff_list, self.wbasis, self.mode)
                return x

        else:
            coeff_dict = array_to_pywt_dict(coeff, self.size_list, self.axes)
            x = pywt.idwtn(coeff_dict, self.wbasis, self.mode, self.axes)
            return x
...
class WaveletTransformAdjoint(Operator):

    """Discrete adjoint of wavelet tranform between discrete Lp spaces."""

    def __init__(self, ran, wbasis, mode, nscales):
        """Initialize a new instance.

         Parameters
        ----------
        ran : `DiscreteLp`
            Range of the wavelet transform (the "image domain").
            The exponent :math:`p` of the discrete :math:`L^p`
            space must be equal to 2.0.
...
-dwt-max-level>`_ .
        ...
        if ran.ndim not in [1, 2, 3]:
            raise NotImplementedError('Dimension of the domain {} not 1, '
                                      '2 or 3'.format(len(ran.ndim)))
        ...
        if self.nscales > max_level:
            raise ValueError('Cannot use more than {} scaling levels, '
                             ...
                             'can be computed using pywt.dwt_max_level '
                             ''.format(max_level, self.nscales))
        self.size_list = coeff_size_list(ran.shape, self.wbasis, self.mode,
                                         self.nscales, axes=None)

        multiplicity = {1: 1, 2: 3, 3: 7}
        dom_size = (np.prod(self.size_list[0]) +
                    sum(multiplicity[ran.ndim] * np.prod(shape)
                        for shape in self.size_list[1:-1]))
    ...
    def _call(self, coeff):
        ...
        """
        x = correct_adjoint_of_wavelet_transform(self.range.shape, self.wbasis,
                                                 self.mode, coeff,
                                                 self.nscales)
        return x
    ...
    def adjoint(self):
        ...
        if self.is_orthogonal:
            return WaveletTransform(dom=self.range, wbasis=self.wbasis,
                                    mode=self.mode, nscales=self.nscales,
                                    axes=self.axes)
        elif self.wbasis.name.startswith('bior'):
            adjoint_name = self.wbasis.name.replace('bior', 'rbio')
