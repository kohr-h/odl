...
from odl.functional.functional import Functional, FunctionalQuadraticPerturb
from odl.functional.prox_ops import (
    proj_simplex, prox_box_constraint, prox_const_func, prox_convex_conj,
    prox_convex_conj_kl, prox_convex_conj_kl_cross_entropy,
...
class IdentityFunctional(ScalingFunctional):
    ...
    See Also
    --------
    odl.operator.basic_ops.IdentityOperator
    """
...
class L2NormSquared(Functional):
    ...
    def proximal(self):
        ...
        See Also
        --------
        odl.functional.prox_ops.prox_l2_squared :
            `proximal factory` for the squared L2-norm.
        """
        return prox_l2_squared(space=self.domain)
...
class Huber(Functional):
    ...
    def __init__(self, space, gamma):
        ...
        >>> x = gamma / 2 * space.one()
        >>> f = 1 / (2 * gamma) * fn.L2NormSquared(space)
        >>> abs(huber_norm(x) - f(x)) < tol
        True
        ...
        >>> huber_norm = fn.Huber(space, gamma=0)
        >>> l1_norm = fn.L1Norm(space)
        ...
        >>> x = odl.phantom.white_noise(space)
        >>> huber_norm = fn.Huber(space, gamma=0)
        >>> l1_norm = fn.GroupL1Norm(space, 2)
        >>> abs(huber_norm(x) - l1_norm(x)) < tol
        True
        """
    ...
    def _call(self, x):
        ...
        if isinstance(self.domain, ProductSpace):
            norm = op.PointwiseNorm(self.domain, 2)(x)
        else:
    ...
    *Collaborative Total Variation: A General Framework for Vectorial TV
    ...
    def __init__(self, space, outer_exp=1, singular_vector_exp=2):
        ...
        self.pwisenorm = op.PointwiseNorm(
            self.domain[0], exponent=singular_vector_exp
        )
...
class KullbackLeibler(Functional):
    ...
    def __init__(self, space, prior=None):
        ...
        >>> space = odl.rn(3)
        >>> prior = 3 * space.one()
        >>> func = fn.KullbackLeibler(space, prior=prior)
        >>> func(prior)
        0.0
