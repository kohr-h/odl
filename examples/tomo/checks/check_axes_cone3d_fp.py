"""Cone 3D example for checking that orientations are handled correctly.
...
proj_data.show(indices=[1, slice(None), slice(None)],
               title='Projection at 90 degrees, axis = [0, 0, 1]')
fig, ax = plt.subplots()
...
proj_data.show(indices=[0, slice(None), slice(None)],
               title='Projection at 0 degrees, axis = [0, 1, 0]')
fig, ax = plt.subplots()
...
proj_data.show(indices=[2, slice(None), slice(None)],
               title='Projection at 180 degrees, axis = [1, 0, 0]')
fig, ax = plt.subplots()
